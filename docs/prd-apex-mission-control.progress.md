...
## Performans — stres testi + PTY output coalescing (2026-06-20, v0.0.2 üstüne)
- **Kullanıcı belirtisi:** v0.0.1'de donma+kilitlenme; v0.0.2'de (WebGL+debounce var) hâlâ terminal tuş gecikmesi. "Framework (Tauri/WKWebView) suçlu mu?" sorusu.
- **Stres testi aracı (dev-only, `VITE_APEX_PERF=1`):** `src/perf/harness.ts` + `main.tsx` `<Profiler>` + `App.tsx` dev hook (`window.__apexPerf` ile sentetik branch/terminal yükü). Kendi kendine ölçüyor (insan yazımı gerekmez). Metrikler: event-loop lag (tuş gecikmesinin doğrudan ölçüsü), React commit süresi, frame jank (+ hidden/focus geçerlilik bayrağı), Tauri IPC gidiş-dönüş, ve **gerçek PTY keystroke→echo gecikmesi** (idle / 500-branch+4-terminal yük / **çıktı seli altında**). Rapor → `/tmp/apex-perf-report.json`.
- **Kök neden (3 koşumda kanıtlandı, hipotez çürütüldü):** Suçlu Tauri IPC DEĞİL (gidiş-dönüş 0.6ms), React re-render DEĞİL (500 branch'te bile commit ≤7ms, monolitik-App teorisi profiler ile çürüdü), JS ana-iş-parçacığı bloğu DEĞİL (loop-lag yük altında ≤9ms). **Gerçek suçlu:** `pty.rs` reader her 8KB okumada bir Tauri `Channel` event'i yolluyordu → yoğun çıktılı terminal (claude TUI redraw) ~1900 event/sn ile WKWebView'in tek-iş-parçacıklı native→JS mesaj kuyruğunu doyuruyor → kullanıcının kendi tuş echo'su selin arkasına kuyruklanıyor. Ölçüm: çıktı seli altında echo p95 **405ms**, max **453ms** (idle'da p50 1-3ms).
- **Düzeltme (`pty.rs`, pragmatik tek-dosya):** İki aşamalı okuma — reader thread blocking read'leri mpsc'ye iter; coalescer thread drained-burst başına **tek** Channel event yollar (tek tuş hemen flush olur → idle gecikme yok; sel kendi kendine MAX_BATCH=256KB'a kadar batch'lenir → ~1900 event/sn → ~60-70). 
- **Doğrulama (yeniden derlenen binary, Golden Rule §2):** Sel daha ağırken (18.5 MB/s) echo avg 34→**12.7ms**, p95 405→**235ms**, max 453→**246ms**. cargo clippy -D warnings temiz, 3 pty testi geçti.
- **Kalan kuyruk (~235ms p95):** Sel-dışı "yük altında"da da var (500 sentetik branch'i 1sn'de yeniden render eden stres artefaktı; büyük DOM repaint event teslimini erteliyor). Gerçek kullanımda branch sayısı <40 → bu kuyruk çok daha küçük olmalı (kanıtlanmadı, çıkarım). Gerekirse derin çözüm: Channel'ı binary encode et (bytes şu an JSON sayı dizisi olarak ~4x şişiyor) — daha büyük iş.
- **Bilinen kısıt:** Test dev-mode + sentetik. Gerçek "claude çalışırken terminalde yazma" akışı imzalı build'de (v0.0.3) kullanıcı tarafından doğrulanmalı.

### Devam — binary channel + backpressure (2026-06-20/21)
- **2. düzeltme — binary Channel:** `pty.rs` data event'i artık `InvokeResponseBody::Raw(bytes)` (JSON enum değil). Tauri ≥1KB raw payload'ı binary fetch yolundan ArrayBuffer olarak yolluyor → JSON sayı-dizisi ~4x şişme gitti. Frontend (`Terminal.tsx`, `harness.ts`) `msg instanceof ArrayBuffer` ile ayırıyor; exit = küçük JSON `{"type":"exit"}`. (Kaynak doğrulandı: `tauri-2.11.3/src/ipc/channel.rs`, MAX_RAW_DIRECT_EXECUTE_THRESHOLD=1024.)
- **KRİTİK BULGU — backpressure yok (OOM/runaway):** Binary değişiklik Rust send tarafını hızlandırınca gizli bir bug ortaya çıktı: reader PTY'yi sınırsız kuyruğa boşaltıyor, coalescer'ın async `ch.send`'i hiç bloklamıyordu → yüksek-çıktılı süreç (`yes`, claude redraw) belleği sınırsız büyütüyor. **Stres testinde uygulama 213% CPU + 9.3GB RAM'e fırladı.** Üretimde herhangi bir runaway-output komutu uygulamayı OOM edebilirdi. (Bug orijinal kodda da vardı; per-8KB async eval kuyruğu sınırsızdı.)
- **3. düzeltme — backpressure (`pty.rs`):** `mpsc::sync_channel(256)` (bounded ~2MB) + coalescer her flush'tan sonra `sleep(8ms)`. Tüketici geri kalınca reader `tx.send`'de bloklar → OS PTY buffer dolar → child write() bloklar → runaway throttle olur. Bellek sınırlı, çıktı tavanı ~16MB/s (gerçek terminal asla yaklaşmaz). Tek tuş echo'su flush'tan ÖNCE gittiği için idle gecikme artmaz.
- **Doğrulama (v5b, 3GB memory-watchdog'lu koşum, Golden Rule §2):**
  | metrik (echo) | v3 (fix yok) | v4 (coalesce) | v5b (binary+backpressure) |
  |---|---|---|---|
  | flood p95 | 405ms | 235ms | **3ms** |
  | flood avg | 34ms | 12.7ms | **7.7ms** |
  | under_load p95 | 213ms | 235ms | **4ms** |
  | echoesReceived | 40/40 | 40/40 | **40/40** (terminal ArrayBuffer kontratı sağlam) |
  | bellek | — | — | **runaway YOK** (watchdog sessiz) |
  - IPC 0.5ms, loop-lag ≤10ms, frontend 14/14 + backend 14 test geçti, clippy -D warnings temiz.
- **Kalan max outlier (~200-300ms, 1/40):** idle'da bile var → ortam kaynaklı (GC/compositor) tek-seferlik hiccup, sel mekanizması değil; p95 artık ≤5ms. worstFrameMs (~260ms) = statik UI'da rAF seyrek çağrılması artefaktı (loop-lag düz → gerçek jank değil).
- **Durum:** Düzeltmeler kodda, LOCAL (commit edilmedi — kullanıcı kararı bekliyor). Gerçek "claude çalışırken yazma" akışı imzalı v0.0.3'te kullanıcı doğrulamalı.

## Performans — komutları main thread'den çıkar (async commands) — v0.0.4 (2026-06-21)
- **Kalan belirti (v0.0.3 sonrası kullanıcı):** sayfa geçişi 1-2 sn donma; terminalde her 2-3 kelimede ~0.3 sn donma. + Kullanıcı sordu: "hangi dille yazılmalı, framework mı?"
- **Araştırma (2 paralel subagent, kaynaklı):** Benzer projeler (Alacritty/WezTerm/Ghostty/Zed native GPU; VS Code Electron+xterm.js). Sonuç: **rewrite GEREKMİYOR**. WKWebView'in gerçek kusurları var (rAF 60fps cap — WebKit #173434; → `tauri-plugin-macos-fps`) ama xterm WebGL (zaten var) ana kaldıraç. Native GPU = çok-aylık rewrite, gerekçesiz. Electron = daha akıcı ama 2x RAM, ancak app-code düzeltilmeden geçiş erken.
- **Kök neden (Tauri kaynağında doğrulandı):** 21 Tauri komutunun TAMAMI `sync fn` → `#[tauri::command]` = `ExecutionContext::Blocking` = **main thread'de koşuyor** (`tauri-macros-2.6.3/command/wrapper.rs:50,266`). git/`claude`/fs alt-süreçleri main thread'i bloke edip UI'yı donduruyordu. Sayfa geçişi: `pm_status` workspace başına 6-7 git süreci. Typing: 3sn'de `list_agent_sessions` → `claude agents --json` (~240ms cold-start) main thread'de.
- **Fix:** Ağır komutları off-main-thread'e taşı — `#[tauri::command]` → `#[tauri::command(async)]` (sync fn + async attr = `sync_threadpool` execution, `wrapper.rs:264`). 17 komut: pm.rs(5), fs.rs(7), agents.rs(3), history.rs(1), metrics.rs(1). PTY komutları SYNC kaldı (keystroke düşük-gecikme). Sadece attribute değişti (17 ins / 17 del, body'ye dokunulmadı).
- **Doğrulama (v6 harness, yeni `heavyCommandProbe`, Golden Rule §2):** `list_agent_sessions` çağrısı **240ms sürüyor ama o sırada event-loop lag p95 5ms** = main thread SERBEST (fix öncesi 240ms blok = donma). Yan etki: worstFrameMs **250-350ms → 14-28ms**, jankFrames **3-5 → 0**, flood echo max **246ms → 4ms**. (O ~250ms frame stall'ların bir kısmı sync poll'ların main-thread bloğuymuş — "rAF artefaktı" tahminim kısmen yanlıştı.) clippy -D warnings temiz, frontend 14 + backend 14 test geçti.
- **Bilinen kısıt:** Harness komutları doğrudan invoke ediyor; gerçek workspace ile navigasyon kullanıcı tarafında doğrulanmalı. Opsiyonel ek optimizasyon (yapılmadı, gerek görülmedi): frontend'de `fs-changed`→fsTick debounce + QueuePage invoke batch (artık donmuyor ama arka planda fazla git süreci).
