...
export default function BranchDAG({
  ...
  nodes.forEach((node) => {
    // Parent resolver for mock visual relationship flow
    if (node.name !== "main") {
      let parentNode = rootNode;

      // release branches split from main
      if (node.name.startsWith("release/")) {
        parentNode = rootNode;
      }
      // feature/redis-telemetry forks off feature/stripe-webhooks dynamically to look cool
      else if (node.name === "feature/redis-telemetry") {
        const stripe = nodes.find((n) => n.name === "feature/stripe-webhooks");
        if (stripe) parentNode = stripe;
      }
      // other feature branches split from main
      else {
        parentNode = rootNode;
      }
